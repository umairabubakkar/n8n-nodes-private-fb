...
export class FacebookPagePublisher implements INodeType {
	...
	methods = {
		loadOptions: {
			async getPages(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				try {
					...
					if (!userAccessToken) {
						...
						if ((credentials as any).oauthTokenData && typeof (credentials as any).oauthTokenData === 'object') {
							const tokenData = (credentials as any).oauthTokenData;
			...
			async getPostIds(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				try {
					const credentials = await this.getCredentials('facebookOAuth2Api');
			...
			async getRecipientIds(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				try {
					const credentials = await this.getCredentials('facebookOAuth2Api');
					const pageId = this.getCurrentNodeParameter('pageId') as string;
					...
					const recipients: INodePropertyOptions[] = [];
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				// Try to get token from oauthTokenData if available
				if (!userAccessToken && (credentials as any).oauthTokenData && typeof (credentials as any).oauthTokenData === 'object') {
					const tokenData = (credentials as any).oauthTokenData;
					userAccessToken = tokenData.access_token || tokenData.accessToken;
				}
				...
				if (['post', 'comment', 'media', 'event'].includes(resource)) {
					...
					const targetPage = pageResponse.data.find((page: any) => page.id === pageId);
					if (!targetPage) {
						throw new Error(`Page with ID ${pageId} not found or you don't have access to it. Available pages: ${pageResponse.data.map((p: any) => p.name).join(', ')}`);
					}
				...
				if (resource === 'post') {
					if (operation === 'createTextPost') {
						...
						const backgroundPostMessage = this.getNodeParameter('backgroundPostMessage', i) as string;
						const textFormatPresetId = this.getNodeParameter('textFormatPresetId', i) as string;
						const customPresetId = this.getNodeParameter('customPresetId', i, '') as string;
						...
						const linkPostMessage = this.getNodeParameter('linkPostMessage', i) as string;
						const linkPostUrl = this.getNodeParameter('linkPostUrl', i) as string;
						...
						const published = this.getNodeParameter('published', i, true) as boolean;
						...
						const videoInputType = this.getNodeParameter('videoInputType', i) as string;
						const videoUrl = this.getNodeParameter('videoUrl', i, '') as string;
						const videoBinaryProperty = this.getNodeParameter('videoBinaryProperty', i, 'data') as string;
						...
						const linkMessage = this.getNodeParameter('linkMessage', i, '') as string;
				...
				// ===== COMMENT OPERATIONS =====
					...
					if (operation === 'getComments') {
						...
						const commentPostId = this.getNodeParameter('commentPostId', i) as string;
						const commentMessage = this.getNodeParameter('commentMessage', i) as string;
						...
						const commentPostId = this.getNodeParameter('commentPostId', i) as string;
						const imageInputType = this.getNodeParameter('imageInputType', i, 'url') as string;
						const commentMessage = this.getNodeParameter('commentMessage', i, '') as string;
				...
				// ===== MESSAGE OPERATIONS =====
					...
					if (operation === 'sendTextMessage') {
						...
						const recipientId = this.getNodeParameter('recipientId', i) as string;
						const messageText = this.getNodeParameter('messageText', i) as string;
						const quickReplies = this.getNodeParameter('quickReplies', i, []) as any[];
				...
				// ===== INSIGHTS OPERATIONS =====
					...
					if (operation === 'getPageInsights') {
						...
						const insightsPostId = this.getNodeParameter('insightsPostId', i) as string;
				...
				// ===== EVENT OPERATIONS =====
					...
					if (operation === 'createEvent') {
						...
						const eventStartTime = this.getNodeParameter('eventStartTime', i) as string;
